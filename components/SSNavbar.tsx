...
import { getAuthenticatedUser } from '@/lib/supabase/optimized-client'
...
export default function SSNavbar() {
  ...
  useEffect(() => {
    getAuthenticatedUser().then(setUser).catch(() => {})
  }, [])
  ...
  return (
    <>
      <nav style={navStyle}>
        <Link href="/">
          <Image src="/shameless-logo.png" alt="Simply Shameless" width={120} height={38} style={{ height: 38, width: 'auto' }} priority />
        </Link>

        {/* Desktop */}
        <div style={{ display: 'flex', gap: 40, alignItems: 'center' }} className="hidden md:flex">
          {[['Events', '/events'], ['Shop', '/shop'], ['About', '/#about']].map(([label, href]) => (
